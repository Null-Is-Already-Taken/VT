...
namespace SimblendTools
{
    public class AudioEditor : EditorWindow
    {
        ...
        [MenuItem("Assets/Open in Simblend Audio Editor", true)]
        private static bool ValidateOpenInAudioEditor()
        {
            return Selection.activeObject is AudioClip;
        }

        [MenuItem("Assets/Open in Simblend Audio Editor", false, 2000)]
        private static void OpenInAudioEditor()
        {
            var clip = Selection.activeObject as AudioClip;
            if (clip == null) return;

            // Open the window and pass the clip
            var window = GetWindow<AudioEditor>("Audio Editor");
            window.InitWithClip(clip);
            window.Focus();
        }

        // Add this method to your AudioEditor class
        public void InitWithClip(AudioClip clip)
        {
            audioClip = clip;
            startTrim = 0f;
            endTrim = clip.length;
            fadeStartDuration = 0f;
            fadeEndDuration = 0f;
            isAudioAdded = true;
            ...
            if (audioClip != null)
            {
                ...
                if (waveformTexture != null)
                {
                    ...
                    if (isPlaying)
                    {
                        ...
                        Rect playheadRect = new(playheadPosition, GUILayoutUtility.GetLastRect().y, 2, waveformHeight);
                ...
                GUILayout.Label("New Clip Name (optional):");
                newClipName = GUILayout.TextField(newClipName);

                if (GUILayout.Button("Trim, Fade, and Save"))
                {
                    TrimAndFadeAudioClip(newClipName);
                }
            }
            else
            {
        ...
        private void TrimAndFadeAudioClip(string newClipName = "")
        {
            if (audioClip == null)
            {
            ...
            // Use newClipName if provided, otherwise default to original + "_EDITED"
            string clipName = string.IsNullOrEmpty(newClipName)
                ? fileNameWithoutExtension + "_EDITED"
                : newClipName;

            // Create a new path for the edited audio
            string newPath = Path.Combine(directory, clipName + ".wav");

            // Create trimmed and faded AudioClip
            AudioClip trimmedClip = TrimAndFadeClip(clipName, audioClip, startTrim, length, fadeStartDuration, fadeEndDuration);
            if (trimmedClip != null)
            {
        ...
        private AudioClip TrimAndFadeClip(string newClipName, AudioClip clip, float startTime, float length, float fadeStartDuration, float fadeEndDuration)
        {
            ...
            AudioClip newClip = AudioClip.Create(newClipName, trimmedData.Length / clip.channels, clip.channels, clip.frequency, false);
